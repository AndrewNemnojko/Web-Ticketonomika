...
import { getRewardsHistory, getTodayReward, isBoxAvailable } from '../../../mocks/rewards.mock';
...
export class DrewardService {
  ...
  public async getRewardsHistory(): Promise<Reward[]> {
    ...
    localStorage.setItem(this.rewardsHistoryKey, JSON.stringify(this.rewardsCache));
    return this.rewardsCache;
  }
  ...
  public async OpenBox(): Promise<{ rewards: Reward[]; winningIndex: number }> {
  ...
  private generateRewardsList(realReward: Reward): { rewards: Reward[]; winningIndex: number }{
